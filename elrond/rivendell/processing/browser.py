...
def process_browser(
    ...
    if not os.path.exists(
        ...
        with open(
            ...
            if artefact.endswith("/Edge/History") or artefact.endswith(
                "/chrome/History"
            ):
                cursor_items = (
                    ...
                    .cursor()
                    .execute(
                        "SELECT urls.id, urls.url, urls.title, urls.visit_count, urls.last_visit_time FROM urls;"
                    )
                )
                cursor_visit = (
                    sqlite3.connect(artefact)
                ...
                # cursor_downloads = sqlite3.connect(artefact).cursor().execute("SELECT downloads.url, downloads.full_path, downloads.start_time, downloads.end_time, downloads.received_bytes, downloads.total_bytes FROM downloads;")
                # downloaditems = cursor_downloads.fetchall()
                ...
                pass
            for eachhist in bwsrhist:
                if artefact.endswith("/Edge/History") or artefact.endswith(
                    "/chrome/History"
                ):
                    if str(eachhist)[1:-1].split(",")[-1] != "None":
                        bwsrtime = str(
                            datetime.fromtimestamp(
                                float(str(eachhist)[1:-1].split(",")[-1][:-8])
                            )
                        )
                    else:
                        bwsrtime = "0"
                    ...
                    if str(eachhist)[1:-1].split(",")[-1] != "None":
                        bwsrtime = str(
                            datetime.fromtimestamp(
                                float(str(eachhist)[1:-1].split(",")[-1][:-6])
                            )
                        )
                    else:
                        bwsrtime = "0"
                    ...
                    bwsrtime = str(
                        datetime.fromtimestamp(
                            float(
                                int(str(eachhist).split(",")[-1].split(".")[0])
                                + 978307200
                            )
                        )
                    )
