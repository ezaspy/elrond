...
def compare_iocs(
    ...
    for iocfile in iocfiles:
        if os.path.exists(iocfile.split(": ")[0]):
            with open(iocfile.split(": ")[0], "r") as readfid:
                lines_iocs, current = (
                    {},
                    round(
                ...
                if current != previous and str(current).split(".")[0] != "100":
                    print(
                        ...
                        if len(iocs) > 1:
                            for eachioc in iocs:
                                if (
                                    ...
                                    or (len(eachioc) > 19 and eachioc.endswith("="))
                                ):
                                    with open(
                                        "/opt/elrond/elrond/rivendell/analysing/ioc_exclusions"
                                    ) as ioc_exclusions:
                                        for eachline in ioc_exclusions:
                                            if eachioc != eachline:
                                                iocfiletimes = "{},{},{}".format(
                                                    str(
