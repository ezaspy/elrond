...
def search_keywords(verbosity, output_directory, imgs, keywords):
    print(
    ...
    for each in imgs:
        ...
        for kwsr, _, kwsf in os.walk(mnt):
            ...
            for kwsfile in kwsf:
                try:
                    if os.stat(
                        os.path.join(kwsr, kwsfile)
                    ).st_size > 0 and not os.path.islink(os.path.join(kwsr, kwsfile)):
        ...
        with open(keywords[0], "r") as keywordsfile:
            for eachkeyword in keywordsfile:
                if verbosity != "":
                    print(
                        "     Searching for Keyword '{}'...".format(eachkeyword.strip())
                    )
                else:
                    pass
