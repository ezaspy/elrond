...
def collect_recover_files(
    output_directory, verbosity, mnt, img, vssimage, collectfiles, recover, auto
):
    ...
    if os.path.exists(
    ...
    ) or os.path.exists(output_directory + img.split("::")[0] + "/files/documents"):
        print(
            "\n    \033[1;33mExtracting embedded files from recovered files {}...\033[1;m".format(
                vssimage
            )
        )
        ...
        if len(os.listdir(recoverpath)) > 0:
            for eachrecfile in os.listdir(recoverpath):
                if (
                    not eachrecfile.endswith(".pdf")
                    and not eachrecfile.endswith(".rtf")
                    and not eachrecfile.endswith(".ott")
                    and not eachrecfile.endswith(".odt")
                    and not eachrecfile.endswith(".ods")
                    and not eachrecfile.endswith(".odg")
                ):
                    filezip, ziptmp = (
                        ZipFile(recoverpath + "/" + eachrecfile),
                        recoverpath + "/" + eachrecfile[:-5],
                    )
                                ...
                                for _, _, contentfiles in os.walk(
                                    ...
                                    for contentfile in contentfiles:
                                        if not contentfile.endswith(
                                            ".xml"
                                        ) and not contentfile.endswith(".rels"):
                                            entry, prnt = "{},{},{},{}\n".format(
                                                datetime.now().isoformat(),
                                                vssimage.replace("'", ""),
    ...
    if os.path.exists(output_directory + img.split("::")[0] + "/files/scripts"):
        ...
        if len(os.listdir(recoverpath)) > 0:
            for eachrecfile in os.listdir(recoverpath):
                with open(
                    os.path.join(recoverpath, eachrecfile), encoding="ISO-8859-1"
                ) as scriptfile:
                    lineno = 0
                    for line in scriptfile:
                        obfuscation_evidence = re.findall(
                            r"((?:(?:\+|\()0x\d+\+)|_xor\(| xor |CharCode|\"\+\")", line
                        )
